...
def verify_submission(task_id: str, gene, policy=None, *, steps: int = 900) -> dict:
    """Independently verify a submitted (gene, controller) against a frozen task by RE-RUNNING it in physics and
    applying the honesty gate. Returns ``{task, verified_pass, failure_mode, metrics, summary}`` — the arm's own
    claim is never consulted. ``policy`` is a MorphPolicy (locomotion) / controller handle; ``None`` = the
    scripted/default controller (an honest floor)."""
    ...
    return {"task": task_id, "verified_pass": art["verdict"] == "pass", "failure_mode": art["failure_mode"],
            "metrics": art["metrics"], "summary": art["summary_text"]}
