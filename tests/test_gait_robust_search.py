...
def test_the_fit_publishes_the_per_parameter_margin_and_the_rollout_cost(monkeypatch):
    ...
    monkeypatch.setattr(GS, "evaluate_gait", fake)
    # ``deadline`` and ``eval_cap`` are accepted and ignored: the real ``_one_search`` takes the build's shared
    # gait-fit clock (2026-08-10) and its shared evaluation ledger (2026-08-12), and a double missing the first
    # raised TypeError inside ``fit_gait_for_body``'s own except, turning this assertion about the ROBUSTNESS
    # MARGIN into an error payload. Nothing here is budgeted -- the suite pins the clock off (tests/conftest.py)
    # and sets no evaluation cap, so a margin never becomes a function of machine speed or of a ceiling.
