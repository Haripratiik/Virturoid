...
- Fits each joint's viscous damping, reflected inertia and dry friction with a **confidence interval, not a point estimate**, and applies only what survives two refusals: parameters the experiment could not actually load are reported as unidentified rather than guessed, and a fit that does not measurably improve how the simulator tracks your log is withheld from the model entirely. Every applied calibration is reversible in one call and carries the prior it replaced. Two limits worth stating plainly. The tracking gate is **necessary but not sufficient**, and we measured where it fails: a systematic **torque-scale error** — a wrong gear ratio, torque constant or gearbox efficiency, none of them exotic — is partly absorbable by the parameters we do fit, so a 20% error still improves tracking by 1.507x and **passes**. The gate cannot be fixed by raising its threshold, because a correctly-specified fit has been measured at 1.126x, below a misspecified one at 1.753x; the populations overlap. So every calibration verdict, pass or fail, now ships a `what_this_gate_does_not_catch` field naming the classes it cannot see. Second, the three joint parameters are **written into your model**, but the identified actuation delay is **only reported** — MuJoCo has no transport delay and every actuator compiles at `dyntype=none`, so the delay is used to score the tracking gate rather than shipped with your twin, and the actuator-fidelity level that would require it stays correctly blocked.
